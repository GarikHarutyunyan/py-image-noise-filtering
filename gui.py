...
IMAGE_FOLDER = os.path.join(os.path.dirname(__file__), 'noisy')
ORIGINAL_FOLDER = os.path.join(os.path.dirname(__file__), 'original')

def resize_image(image, width=160):
...
def display_images(images, titles, options):
    ...
    for i in range(len(images)):
        ...
        title_label.grid(row=(i // 8) * 2, column=i % 8, padx=5, pady=(0, 5))

        label = Label(frame, image=image_tk)
        label.image = image_tk
        label.grid(row=(i // 8) * 2 + 1, column=i % 8, padx=5, pady=(0, 5))
        
        get_option = options[i];
        if get_option is not None:
            get_option().grid(row=(i // 8) * 2+2, column=i % 8, padx=5, pady=(0, 5))


def apply_transformations(selected_image_name):
    global frame

    selected_image_path = os.path.join(IMAGE_FOLDER, selected_image_name)
    original_image_path = os.path.join(ORIGINAL_FOLDER, selected_image_name)
    ...
    images = [original_image, selected_image, box_filtered, gaussian_filtered, median_filtered, laplacian_sharpened, unsharp_image, sobel_gradient_image]
    titles = ['Original', 'Noisy', 'Box Filter', 'Gaussian Filter', 'Median Filter', 'Laplacian Sharpening', 'Unsharp Masking', 'Sobel Gradient']
...
def on_selection_change(event):
    ...
    apply_transformations(selected_image_name)
